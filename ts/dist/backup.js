...
const DownloadArea = cc("div", {
    children: [
        m("p").text("点击下面的链接（或右键点击后选择“另存为”）可下载数据库文件（已加密的数据）："),
        m("p").append(util
            .LinkElem("/api/download-backup", { text: "sqlite database file" })
            .attr({ downlaod: "db-mima.sqlite" })),
    ],
});
$("#root").append(m(NaviBar).addClass("my-3"), m(Loading).addClass("my-3"), m(Alerts), m(DownloadArea).addClass('my-5').hide(), m(GotoSignIn).addClass("my-5").hide());
init();
function init() {
    checkSignIn();
}
