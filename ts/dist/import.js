...
const NaviBar = cc("div", {
    children: [
        ...
        m("p").append("本页专用于导入 mima-web 的数据。(", util.LinkElem("https://github.com/ahui2016/mima-web", {
            text: "mima-web",
            blank: true,
        }), " 是本程序的前身)"),
...
const UploadForm = cc("form", {
    children: [
        ...
        m(UploadBtn).on("click", (e) => {
            ...
            const files = FileInput.elem()[0].files;
...
$("#root").append(m(NaviBar).addClass("my-3"), m(Loading).addClass("my-3"), m(Alerts), m(GotoSignIn).addClass("my-3").hide(), m(UploadForm).addClass("my-3").hide());
init();
function init() {
    checkSignIn();
}
