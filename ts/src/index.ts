...
function MimaItem(mima: util.Mima): mjComponent {
  const self = cc('div', {id:mima.ID,classes:'MimaItem', children:[
    m('div').addClass('MimaTitleArea').append(
      span(`[id: ${mima.ID}]`).addClass('text-grey'),
      span('').addClass('MimaLabel ml-2').hide(),
      util.LinkElem('/public/edit.html?id='+mima.ID,{text:mima.Title,blank:true}).addClass('ml-2'),
    ),
    m('div').append(
      span('username: ').addClass('text-grey'), mima.Username,
      span('password: ').addClass('text-grey ml-2'),
      util.LinkElem('#',{text:'copy'}),
    ),
  ]});
  self.init = () => {
    if (mima.Label) {
